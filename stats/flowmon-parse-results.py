...
def getCustomStats(protocol, network_size):
    level = 0
    for event, elem in ElementTree.iterparse(open("final/%s.custom.5.%d" % (protocol, network_size)), events=("start", "end")):
        if event == "start":
            level += 1
        if event == "end":
            level -= 1
...
class Flow(object):
    ...
    def __init__(self, flow_el):
        ...
        if self.rxPackets:
            self.delayMean = float(flow_el.get('delaySum')[:-2])/1000000000000
            self.jitterMean = float(flow_el.get('jitterSum')[:-2])/1000000000000
...
def main(argv):
    ...
    for protocol in protocols:
        for network_size in network_sizes: 
            for event, elem in ElementTree.iterparse(open("final/%s.flomonitor.5.%d" % (protocol, network_size)), events=("start", "end")):
                if event == "start":
                    level += 1
                if event == "end":
                    level -= 1
